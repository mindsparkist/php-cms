<?php
    $page = 'doctors';
    include 'partials/header.php';
?>
        <!-- Container -->
    <body class="content_body">
    <div class="container" style="margin-top:25px;">
      <form action="" method="post" enctype="multipart/form-data">
        <!-- title1 -->
        <div class="form-group">
          <label for="">dTitle 1</label>
          <input
            type="text"
            class="form-control"
            id=""
            placeholder="Example input"
          />
        </div>
        <!-- Sub title -->
        <div class="form-group">
          <label for="">sub Title 1</label>
          <input
            type="text"
            class="form-control"
            id=""
            placeholder="Example input"
          />
        </div>
        <!--Background Image1 image  -->
        <div class="form-group">
          <label>Background Image1</label>
          <input type="file" class="form-control" name="Background_Image1" />
        </div>
        <!-- title2 -->
        <div class="form-group">
          <label for="">Title 2</label>
          <input
            type="text"
            class="form-control"
            id=""
            placeholder="Example input"
          />
        </div>
        <!-- img2 -->
        <div class="form-group">
          <label>Background Image2</label>
          <input type="file" class="form-control" name="img2" />
        </div>
        <!-- q1 -->
        <div class="form-group">
          <label for="">Question 1</label>
          <input
            type="text"
            class="form-control"
            id=""
            placeholder="Example input"
          />
        </div>
        <!-- qna -->
        <div class="form-group">
          <label for=""> answer 1</label>
          <textarea
            class="form-control"
            id=""
            rows="3"
          ></textarea>
        </div>
        <!-- q2 -->
        <div class="form-group">
          <label for="">Q2</label>
          <input
            type="text"
            class="form-control"
            id=""
            placeholder="Example input"
          />
        </div>
        <!-- qna2 -->
        <div class="form-group">
          <label for="">Ans2</label>
          <textarea
            class="form-control"
            id=""
            rows="3"
          ></textarea>
        </div>
        <!-- q3 -->
        <div class="form-group">
          <label for="">Q3</label>
          <input
            type="text"
            class="form-control"
            id=""
            placeholder="Example input"
          />
        </div>
        <!-- qn3 -->
        <div class="form-group">
          <label for="">ans3</label>
          <textarea
            class="form-control"
            id=""
            rows="3"
          ></textarea>
        </div>
        <!-- img3 -->
        <div class="form-group">
          <label>Background Image 3 </label>
          <input type="file" class="form-control" name="image" />
        </div>
        <!-- link1 -->
        <div class="form-group">
          <label for="">Q3</label>
          <input
            type="text"
            class="form-control"
            id=""
            placeholder="Example input"
          />
        </div>
        <!-- img4_back -->
        <div class="form-group">
          <label>Background Image 4 </label>
          <input type="file" class="form-control" name="image" />
        </div>
        <!-- img5 -->
        <div class="form-group">
          <label>Background Image5</label>
          <input type="file" class="form-control" name="image" />
        </div>
        <!-- sub_text1 -->
        <div class="form-group">
          <label for="">Title 1</label>
          <input
            type="text"
            class="form-control"
            id=""
            placeholder="Example input"
          />
        </div>
        <!-- sub_txt2 -->
        <div class="form-group">
          <label for="">Title 1</label>
          <input
            type="text"
            class="form-control"
            id=""
            placeholder="Example input"
          />
        </div>
        <!-- sub_txt3 -->
        <div class="form-group">
          <label for="">Title 1</label>
          <input
            type="text"
            class="form-control"
            id=""
            placeholder="Example input"
          />
        </div>
        <!-- link2 -->
        <!-- title3 -->
        <div class="form-group">
          <label for="">Title 1</label>
          <input
            type="text"
            class="form-control"
            id=""
            placeholder="Example input"
          />
        </div>
        <!-- img6_back -->
        <div class="form-group">
          <label>Background Image</label>
          <input type="file" class="form-control" name="image" />
        </div>
        <!-- submit button -->
        <button type="button" class="btn btn-outline-success mt-3">Update</button>
      <br><br>
      </form>
    </div>
      
        <!-- End Container -->
<?php
    include 'partials/footer.php';
?> 
